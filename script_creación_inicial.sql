...
CREATE TABLE M_AL_CUBO.COMPRA(
	...
	DESCUENTO_COMPRA_ID INT, --HAY QUE BORRARLO A ESTE CAMPO!!!
	COMPRA_FECHA DATE,
...
GO
 ...
 SELECT DISTINCT *
  FROM GD_ESQUEMA.MAESTRA m
  jOIN M_AL_CUBO.TIPO_VARIANTE tv on tv.PRODUCTO_VARIANTE_CODIGO = m.PRODUCTO_VARIANTE_CODIGO
JOIN M_AL_CUBO.PRODUCTO_X_VARIANTE pv on tv.VARIANTE_ID_CODIGO = pv.PRODUCTO_X_VAR_ID
    where m.COMPRA_PRODUCTO_CANTIDAD is not null 
...
BEGIN 
  ...
  SELECT DISTINCT (SELECT pv.PRODUCTO_X_VAR_ID FROM M_AL_CUBO.PRODUCTO_X_VARIANTE pv 
 JOIN m_al_cubo.TIPO_VARIANTE tv ON tv.TIPO_VARIANTE_ID = pv.tipo_variante_id
 JOIN m_al_cubo.PRODUCTO p on p.producto_id = pv.PRODUCTO_ID
 WHERE p.PRODUCTO_CODIGO = m.PRODUCTO_CODIGO
 AND m.PRODUCTO_VARIANTE_CODIGO = tv.PRODUCTO_VARIANTE_CODIGO), 
 COMPRA_NUMERO, COMPRA_PRODUCTO_CANTIDAD, COMPRA_PRODUCTO_PRECIO
  FROM GD_ESQUEMA.MAESTRA m
  JOIN M_AL_CUBO.TIPO_VARIANTE tv on tv.PRODUCTO_VARIANTE_CODIGO = m.PRODUCTO_VARIANTE_CODIGO
  JOIN M_AL_CUBO.PRODUCTO_X_VARIANTE pv2 on tv.VARIANTE_ID_CODIGO = pv2.PRODUCTO_X_VAR_ID
  WHERE COMPRA_NUMERO IS NOT NULL
END 
GO
...
BEGIN 
  ...
  SELECT DISTINCT p.PRODUCTO_ID, tv.TIPO_VARIANTE_ID
  FROM GD_ESQUEMA.MAESTRA m 
...
--SELECT DISTINCT P.VENTA_CODIGO, P.VENTA_MEDIO_PAGO, P.CLIENTE_DNI, P.VENTA_CANAL, P.VENTA_MEDIO_ENVIO, P.VENTA_TOTAL, P.VENTA_FECHA FROM GD_ESQUEMA.MAESTRA P
  ...
  SELECT DISTINCT P.VENTA_CODIGO, VMV.MEDIO_PAGO_VENTAS_ID, C.CLIENTE_ID, VC.CANAL_VENTA_ID, P.VENTA_MEDIO_ENVIO, P.VENTA_TOTAL, P.VENTA_FECHA FROM GD_ESQUEMA.MAESTRA P
...
EXEC M_AL_CUBO.MIGRAR_PRODUCTO_X_VARIANTE
