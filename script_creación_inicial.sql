...
IF OBJECT_ID('M_AL_CUBO.FK_TIPO_VARIANTE_ID', 'F') IS NOT NULL
  ALTER TABLE M_AL_CUBO.TIPO_VARIANTE DROP CONSTRAINT FK_TIPO_VARIANTE_ID
GO
...
ALTER TABLE M_AL_CUBO.TIPO_VARIANTE ADD CONSTRAINT FK_TIPO_VARIANTE_ID FOREIGN KEY (PRODUCTO_VARIANTE_CODIGO) REFERENCES M_AL_CUBO.VARIANTE(VARIANTE_ID_CODIGO)
GO
...
BEGIN 
  ...
  SELECT ROW_NUMBER() OVER (ORDER BY CLIENTE_PROVINCIA) PROVINCIA_ID, CLIENTE_PROVINCIA PROVINCIA_NOMBRE FROM GD_ESQUEMA.MAESTRA
	WHERE CLIENTE_PROVINCIA IS NOT NULL
	GROUP BY CLIENTE_PROVINCIA
	UNION
	SELECT ROW_NUMBER() OVER (ORDER BY PROVEEDOR_PROVINCIA) PROVINCIA_ID, PROVEEDOR_PROVINCIA PROVINCIA_NOMBRE FROM GD_ESQUEMA.MAESTRA
...
BEGIN 
  INSERT INTO M_AL_CUBO.CATEGORIA (CATEGORIA_ID, PRODUCTO_CATEGORIA)
	SELECT ROW_NUMBER() OVER (ORDER BY PRODUCTO_CATEGORIA) CATEGORIA_ID, PRODUCTO_CATEGORIA FROM GD_ESQUEMA.MAESTRA
...
BEGIN 
  INSERT INTO M_AL_CUBO.MARCA (MARCA_ID, PRODUCTO_MARCA)
	SELECT ROW_NUMBER() OVER (ORDER BY PRODUCTO_MARCA) MARCA_ID, PRODUCTO_MARCA FROM gd_esquema.Maestra
...
BEGIN 
  INSERT INTO M_AL_CUBO.MATERIAL (MATERIAL_ID, PRODUCTO_MATERIAL)
	SELECT ROW_NUMBER() OVER (ORDER BY PRODUCTO_MATERIAL) MATERIAL_ID, PRODUCTO_MATERIAL FROM GD_ESQUEMA.MAESTRA
...
BEGIN 
  INSERT INTO M_AL_CUBO.MEDIO_ENVIO(ENVIO_ID, ENVIO_DESCRIPCION)
	SELECT ROW_NUMBER() OVER (ORDER BY VENTA_MEDIO_ENVIO) ENVIO_ID, VENTA_MEDIO_ENVIO FROM gd_esquema.MAESTRA
...
BEGIN 
  INSERT INTO M_AL_CUBO.CANAL(CANAL_ID, CANAL_DESCRIPCION)
	SELECT ROW_NUMBER() OVER (ORDER BY VENTA_CANAL) CANAL_ID, VENTA_CANAL FROM GD_ESQUEMA.MAESTRA
