...
AS
SELECT TIEMPO_MES, MEDIO_PAGO_DESCRIPCION, INGRESOS FROM (SELECT ROW_NUMBER() OVER (PARTITION BY T.TIEMPO_MES, MP.MEDIO_PAGO_DESCRIPCION ORDER BY SUM(T.TIEMPO_MES)) E, T.TIEMPO_MES, MP.MEDIO_PAGO_DESCRIPCION, 
	CASE WHEN EXISTS (SELECT * FROM BI_M_AL_CUBO.BI_HECHO_DESCUENTO D JOIN BI_M_AL_CUBO.BI_TIEMPO TI ON TI.TIEMPO_ID = D.ID_TIEMPO  WHERE D.ID_DESCUENTO = ID_MEDIO_PAGO AND TI.TIEMPO_MES = T.TIEMPO_MES)
	...
	- ISNULL((SELECT SUM(DESCUENTO_IMPORTE) FROM BI_M_AL_CUBO.BI_HECHO_DESCUENTO D JOIN BI_M_AL_CUBO.BI_TIEMPO TI ON TI.TIEMPO_ID = D.ID_TIEMPO WHERE ID_DESCUENTO = ID_MEDIO_PAGO AND TI.TIEMPO_MES = T.TIEMPO_MES ),0)
	...
	- ISNULL((SELECT SUM(DESCUENTO_IMPORTE) FROM BI_M_AL_CUBO.BI_HECHO_DESCUENTO D JOIN BI_M_AL_CUBO.BI_TIEMPO TI ON TI.TIEMPO_ID = D.ID_TIEMPO WHERE ID_DESCUENTO = 2 AND TI.TIEMPO_MES = T.TIEMPO_MES),0) END
	 INGRESOS
